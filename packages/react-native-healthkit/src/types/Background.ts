...
/** Breadcrumb entry flushed on the next successful push after a failure. */
export interface SyncBreadcrumb {
  readonly timestamp: string
  readonly reason: string
  readonly path: 'native'
}

/**
 * Shape of the HTTP request body sent to the consumer's endpoint.
 *
 * `clientFailuresSince` is present only when the native engine previously
 * failed to push and has accumulated breadcrumbs — they are flushed on the
 * next successful request so the consumer can observe gaps.
 */
export interface SyncRequestBody {
  readonly records: readonly SyncRecord[]
  readonly clientFailuresSince?: readonly SyncBreadcrumb[]
}
